    ...
    async function onSubmit({ otp }) {
        ...
        try {
            ...
            if (!OTPText.success) {
                ...
                await JwtService.sendOtpToMobileNumber(usercredential.mobileno);
                setLoading(false)
            } else {
                const response = await JwtService.verifyMobileNumberOtp(usercredential.mobileno, sendotp);
                if (response.success !== true) {
                    dispatch(showMessage({ message: 'Incorrect OTP!', variant: 'error' }));
                    setOTPText({ text: 'Resend OTP', success: false });
                    setLoading(false);
                } else {
                    await JwtService.signInWithEmailPassword(usercredential.mobileno, usercredential.password)
                    reset(defaultValues);
                    setLoading(false);
                }
            }
            ...
            <div className="w-full max-w-320 sm:w-320 mx-auto sm:mx-0">
                ...
                {OTPText.success !== false
                    ...
                    <div className="flex flex-col justify-center items-center mt-2 font-medium">
                        ...
                        <Typography>{`(Mobile Number: ${usercredential.mobileno})`}</Typography>
                    </div>
                }
                <form
                    name="registerForm"
