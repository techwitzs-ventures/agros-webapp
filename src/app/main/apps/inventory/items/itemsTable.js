...
function ItemsTable(props) {
  ...
  useEffect(() => {
    if (searchText.length !== 0) {
      setData(
        _.filter(products, (item) => item.productInfo[0].name.toLowerCase().includes(searchText.toLowerCase()))
      );
      setPage(0);
    } else {
      setData(products);
  ...
  function handleClick(item) {
    props.navigate(`/apps/inventory/items/${item.id}`)
  }
                    ...
                    >
                      {n.images.length > 0 ? (
                        <img
                          ...
                          src={_.find(n.productImages[0].images, { isCover: true }).url}
                          alt={n.productInfo[0].name}
                        />
                      ) : (
                        <img
                          className="w-full block rounded"
